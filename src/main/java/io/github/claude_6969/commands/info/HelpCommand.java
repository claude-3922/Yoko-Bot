...
public class HelpCommand extends Command {
    ...
    protected void execute(CommandEvent commandEvent) {
        ...
        for (Command command : Commands) {
            ...
            commandLines[i] = "```nim\nCommand : ;;%s\nAliases : %s\nGuild Only : %s\nDescription : \"%s\"\nUsage : \";;%s %s\"\nCooldown : %s seconds```".formatted(command.getName(), aliasString, command.isGuildOnly() ? "true" : "false", command.getHelp(), command.getName(), command.getArguments(), command.getCooldown());
            i++;
        }
        builder.setText("Prefix is `;;`")
